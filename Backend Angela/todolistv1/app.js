...
// New schema for other items
const listSchema = {
  name: String,
  items: [itemSchema], // items with that of item schema
};
...
// make dynamic route with help of express
app.get("/:customListName", (req, res) => {
  const customListName = req.params.customListName;

  List.find({ name: customListName }, function (err, foundItems) {
    if (!foundItems) {
      // make a new list and then render
      const list = new List({
        name: customListName,
        items: defaultItems,
      });
